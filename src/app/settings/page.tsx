...
import { ArrowLeft, Sun, Moon, Monitor, Crown, ChevronRight, Globe, ChevronDown, Pencil, Check, X, Camera } from "lucide-react";
    ...
    <div className="min-h-screen bg-neutral-50 dark:bg-[#0a0a0a]">
      ...
      <div className="max-w-2xl mx-auto px-6 py-8 space-y-6">
        ...
        <section className="bg-white dark:bg-[#111] border border-neutral-200 dark:border-white/[0.07] rounded-2xl overflow-hidden">
          <div className="px-6 py-4 border-b border-neutral-100 dark:border-white/[0.05]">
            <h2 className="font-semibold text-sm uppercase tracking-wider text-neutral-500 dark:text-white/40">{t.profile}</h2>
          </div>
          <div className="px-6 py-5 flex items-center gap-4">
            ...
            <div className="relative group flex-shrink-0">
              ...
              {(localPhotoURL ?? user?.photoURL) ? (
                <img src={localPhotoURL ?? user?.photoURL!} alt="avatar" className="w-16 h-16 rounded-2xl object-cover shadow-lg" />
              ) : (
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500 to-violet-600 flex items-center justify-center text-white font-bold text-2xl shadow-lg shadow-blue-500/20">
              ...
              {editingName ? (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 bg-neutral-100 dark:bg-white/[0.07] border border-neutral-200 dark:border-white/[0.1] rounded-lg px-3 py-1.5 text-[15px] font-semibold text-neutral-900 dark:text-white outline-none focus:border-blue-500 dark:focus:border-blue-500"
                    placeholder="Your name"
                  />
                  <button onClick={saveName} disabled={nameSaving} className="w-8 h-8 rounded-lg bg-blue-500 flex items-center justify-center hover:bg-blue-600 transition-colors flex-shrink-0">
                    {nameSaving ? <div className="w-3.5 h-3.5 border-2 border-white/40 border-t-white rounded-full animate-spin" /> : <Check className="w-3.5 h-3.5 text-white" />}
                  </button>
                  <button onClick={() => { setEditingName(false); setNameError(""); }} className="w-8 h-8 rounded-lg bg-neutral-100 dark:bg-white/[0.07] flex items-center justify-center hover:bg-neutral-200 dark:hover:bg-white/[0.12] transition-colors flex-shrink-0">
                    <X className="w-3.5 h-3.5 text-neutral-500 dark:text-white/50" />
                  </button>
                </div>
              ) : (
                <div className="flex items-center gap-2 group/name">
                  <p className="font-semibold text-neutral-900 dark:text-white text-[15px] truncate">{getDisplayName(user)}</p>
                  <button
                    onClick={() => { setNameValue(user?.displayName ?? ""); setEditingName(true); setNameError(""); }}
                    className="opacity-0 group-hover/name:opacity-100 w-6 h-6 rounded-md hover:bg-neutral-100 dark:hover:bg-white/[0.08] flex items-center justify-center transition-all flex-shrink-0"
                  >
                    <Pencil className="w-3 h-3 text-neutral-400 dark:text-white/40" />
                  </button>
                </div>
              )}
              {nameError && <p className="text-xs text-red-500 mt-1">{nameError}</p>}
              <p className="text-sm text-neutral-500 dark:text-white/40 mt-0.5 truncate">{user?.email ?? user?.phoneNumber ?? "Not signed in"}</p>
            </div>
          </div>
        </section>
        ...
        {/* Danger Zone */}
        <section className="bg-white dark:bg-[#111] border border-red-200 dark:border-red-500/20 rounded-2xl overflow-hidden">
          <div className="px-6 py-4 border-b border-red-100 dark:border-red-500/10">
            <h2 className="font-semibold text-sm uppercase tracking-wider text-red-400 dark:text-red-400/70">Danger Zone</h2>
          </div>
          <div className="px-6 py-4 flex items-center justify-between gap-4">
            <div>
              <p className="text-sm font-semibold text-neutral-800 dark:text-white/80">Delete Account</p>
              <p className="text-xs text-neutral-400 dark:text-white/30 mt-0.5">Permanently delete your account and all data</p>
            </div>
            <button
              onClick={() => { setShowDeleteConfirm(true); setDeleteInput(""); setDeleteError(""); }}
              className="text-xs font-semibold text-red-500 border border-red-300 dark:border-red-500/30 rounded-lg px-4 py-2 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors shrink-0"
            >
              Delete Account
            </button>
          </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-neutral-200 dark:border-white/[0.08] p-6 w-full max-w-sm shadow-xl">
            <h3 className="text-lg font-bold text-neutral-900 dark:text-white mb-2">Delete your account?</h3>
            <p className="text-sm text-neutral-500 dark:text-white/40 mb-4">
              This will permanently delete all your conversations and account data. This action cannot be undone.
            </p>
            <p className="text-xs font-semibold text-neutral-600 dark:text-white/50 mb-2">Type <span className="text-red-500 font-mono">DELETE</span> to confirm:</p>
            <input
              className="w-full bg-neutral-100 dark:bg-[#111] border border-neutral-200 dark:border-white/[0.08] rounded-xl px-4 py-3 text-sm text-neutral-900 dark:text-white outline-none font-mono mb-4"
              placeholder="DELETE"
              value={deleteInput}
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-neutral-600 dark:text-white/50 bg-neutral-100 dark:bg-white/[0.05] hover:bg-neutral-200 dark:hover:bg-white/[0.08] transition-colors"
              >
                Cancel
              </button>
              <button
